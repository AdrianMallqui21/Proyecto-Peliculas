...
mostrar, añadir = st.tabs(["Visualizar Actor", "Añadir Nuevo Actor"])

with mostrar:
    st.title("Visualizar Todos los Actores")
...
with añadir:
    st.title("Registro de Nuevo Actor")
    nombre_completo = st.text_input("Nombre: *", key="act_nombre")
    apellido_completo = st.text_input("Apellido: *", key="act_apellido")

    if st.button("Enviar", key="btn_actor"):
        errores = []
